...
public class LinkedList : IEnumerable<int>
{
    ...
    {
        // TODO Problem 2
        // If the list is empty or has only one item in it, then set head and tail to null
        if (_head == _tail)
        {
            _head = null;
            _tail = null;
        ...
        {
            _tail.Prev!.Next = null; // Disconnect the current tail
    ...
    public void Remove(int value)
    {
        ...
        while (curr is not null)
        {
            if (curr.Data == value)
            {
                // If the node is the tail, remove it
                if (curr == _tail)
                {
                    RemoveTail();
                }
                ...
                {
                    curr.Prev!.Next = curr.Next; // Bypass the current node by connecting previous to next
                    curr.Next!.Prev = curr.Prev; // Connect the next node's previous to the current node's previous
                }
